...
#include "matrix_omp.h" // omp version

#include "Subfunc.cpp" // subfunction
...
#   pragma omp parallel for
    ...
    for (int i = 0; i < largenumber; i++)
    {
        ...
        for (int j = 2; j <= N - 1; j++)
        {
            flux_L = Conserved2Flux(L.GetRow(j));
            flux_R = Conserved2Flux(R.GetRow(j));
        ...
        for (int j = nghost + 1; j <= N - nghost+1; j++)
            flux.SetRow(j, Roe(R.GetRow(j-1), L.GetRow(j)));
        ...
        if (myfile.is_open())
        {
            for (int j = nghost + 1; j <= N - nghost; j++)
            {
                ...
                myfile << U(j, 1) << "\t";
                myfile << U(j, 2)/U(j, 1) << "\t";
                myfile << ComputePressure(U(j, 1), U(j, 2), U(j, 3)) << "\t";
                myfile << "\r\n";
            }
        }
        else
