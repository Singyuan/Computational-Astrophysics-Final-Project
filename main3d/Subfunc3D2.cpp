    ...
    for (int i = 1; i <= N; i++)
    {
        for (int j = 1; j <= NY; j++)
        {
            for (int k = 1; k <= foo; k++)
            {
                ...
                u = abs(U(2, i, j, k) / U(1, i, j, k));
                v = abs(U(3, i, j, k) / U(1, i, j, k));
                w = abs(U(4, i, j, k) / U(1, i, j, k));
...
matrix RoeZ(matrix L, matrix R)
{
    ...
    amp(2) = dU(2) - u * dU(1);
    amp(3) = dU(3) - v * dU(1);
    amp(4) = (mygamma - 1.0) / pow(a, 2.0) * (dU(1) * (H - pow(w, 2.0)) + w * dU(4) - dU(5) + u * amp(2) + v * amp(3));
    amp(1) = 0.5 / a * (dU(1, 1) * (w + a) - dU(4) - a * amp(4));
    amp(5) = dU(1) - amp(1) - amp(4);
    ...
    matrix flux_L(5);
    matrix flux_R(5);
    flux_L = Conserved2FluxAddressZ(L);
    flux_R = Conserved2FluxAddressZ(R);
